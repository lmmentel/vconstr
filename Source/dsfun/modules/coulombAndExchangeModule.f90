...
!===============================================================================
  ...
  interface getCoulombAndExchange
    module procedure getCoulombAndExchangeEnergies
  end interface

  public :: getCoulombAndExchange 
...
! set the values of Couomb and Exchange energies to 0.0
      ...
      do m=1, bufLength
        ...
        do iperm = 1, np
            ecoul = ecoul + pmo(ij(ii(iperm),jj(iperm)))* &
                    pmo(ij(kk(iperm),ll(iperm)))*buffer(m)
            eexch = eexch + pmo(ij(ii(iperm),ll(iperm)))* &
                    pmo(ij(kk(iperm),jj(iperm)))*buffer(m)
        enddo
...
! multiply off-diagonal elements of pmo with 2.0
    ...
    deallocate(ij)
  end subroutine getCoulombAndExchangeEnergies 
...
end module CoulombAndExchangeModule
